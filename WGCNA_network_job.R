...
par(mfrow = c(1,2))
cex1 = 0.9
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/power_soft_thresh_VST_combined_abiotic.pdf",width = 10,height = 10)
...
#Using KME with Avinash's code:
###Yellow
gs<-colnames(datExpr0)
#  cols<-net[[1]]
cols <- bwModuleColors
names(cols)<-gs #assign each gene into different module
#modulePrefix ="yellow"
#if(kME.threshold>0){#Avinash said it is not neccessary to 
  #if(verbose) cat("using KME values to find genes with high module membership\n")
  # if(is.null(MEs))
  #   MEs <- moduleEigengenes(datExpr, moduleColors)$eigengenes
 
  kme <- signedKME(datExpr = datExpr0, datME = MEs, outputColumnName = NULL)
  head(kme)
  ...
  yellow_top20_gene_list <- yellow_kmes[1:20,]$genes
  
  gs<-kmes$genes[kmes$cols=="yellow"]
  #cols<-cols[kmes$kme>=kME.threshold]
  datExpr = datExpr0[,gs]
  head(datExpr[,1:8])
  ncol(datExpr)
#}#yellow module we have 2579 genes!!!

adj_mat<-adjacency(datExpr,power=8)
###
head(adj_mat[,1:8])
######### Get the top edges of the yellow module:
top.n.edges = 3000
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 2579
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.594857230070154
...
message("number of genes present = ", nrow(adj_mat))#number of genes present = 260
...
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_yellow.pdf",width = 25,height = 25)
...
blue_top20_gene_list <- blue_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="blue"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
#}#yellow module we have 3587 genes!!!
...
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 3587
if(!is.na(top.n.edges) & nrow(adj_mat) >= 100){
  adjacency.threshold = sort(as.numeric(adj_mat), decreasing=T)[(top.n.edges*2)+nrow(adj_mat)]
}
if(nrow(adj_mat) < 100)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.690966673259787
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
...
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_blue.pdf",width = 25,height = 25)
#plot(network, vertex.color="yellow", vertex.label="",vertex.size=2,layout=layout_in_circle(network), edge.arrow.size = 0.2)
#plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=2.5, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
purple_top20_gene_list <- purple_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="purple"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
#}#yellow module we have 291 genes!!!
...
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 291
if(!is.na(top.n.edges) & nrow(adj_mat) >= 100){
  adjacency.threshold = sort(as.numeric(adj_mat), decreasing=T)[(top.n.edges*2)+nrow(adj_mat)]
}
if(nrow(adj_mat) < 100)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.0988610980336536
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 136
...
#results <- blockwiseModules(data, power=6, TOMType="unsigned", networkType="unsigned")
#V(network)$color <- results$colors
V(network)$color <- "purple"
scale_factor <- 0.1
par(mar=c(0,0,0,0))
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_purple.pdf",width = 25,height = 25)
#plot(network, vertex.color="yellow", vertex.label="",vertex.size=2,layout=layout_in_circle(network), edge.arrow.size = 0.2)
#plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=2.5, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
red_top20_gene_list <- red_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="red"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
...
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 857
if(!is.na(top.n.edges) & nrow(adj_mat) >= 100){
  adjacency.threshold = sort(as.numeric(adj_mat), decreasing=T)[(top.n.edges*2)+nrow(adj_mat)]
}
if(nrow(adj_mat) < 100)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.415321452988634
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 274
...
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_red.pdf",width = 25,height = 25)
#plot(network, vertex.color="yellow", vertex.label="",vertex.size=2,layout=layout_in_circle(network), edge.arrow.size = 0.2)
#plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=2.5, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
kmes<-data.frame(genes = gs, cols = cols, kme = kmes)
head(kmes)
tan_kmes <- kmes[kmes$cols=="tan",]
head(tan_kmes)
tan_kmes <-tan_kmes[order(-tan_kmes$kme),]
tan_top20_gene_list <- tan_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="tan"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
#}#yellow module we have 2342 genes!!!
...
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 201
if(!is.na(top.n.edges) & nrow(adj_mat) >= 100){
  adjacency.threshold = sort(as.numeric(adj_mat), decreasing=T)[(top.n.edges*2)+nrow(adj_mat)]
}
if(nrow(adj_mat) < 100)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.0591105896198783
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 154
...
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_tan.pdf",width = 25,height = 25)
#plot(network, vertex.color="yellow", vertex.label="",vertex.size=2,layout=layout_in_circle(network), edge.arrow.size = 0.2)
#plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=2.5, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
brown_top20_gene_list <- brown_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="brown"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
#}#yellow module we have 2721 genes!!!
...
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 2721
if(!is.na(top.n.edges) & nrow(adj_mat) >= 100){
  adjacency.threshold = sort(as.numeric(adj_mat), decreasing=T)[(top.n.edges*2)+nrow(adj_mat)]
}
if(nrow(adj_mat) < 100)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.654174701725079
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 243
...
#results <- blockwiseModules(data, power=6, TOMType="unsigned", networkType="unsigned")
#V(network)$color <- results$colors
V(network)$color <- "brown"
scale_factor <- 0.1
par(mar=c(0,0,0,0))
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_brown.pdf",width = 25,height = 25)
#plot(network, vertex.color="yellow", vertex.label="",vertex.size=2,layout=layout_in_circle(network), edge.arrow.size = 0.2)
#plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=2.5, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
greenyellow_top20_gene_list <- greenyellow_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="greenyellow"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.126521726744246
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 209
...
#results <- blockwiseModules(data, power=6, TOMType="unsigned", networkType="unsigned")
#V(network)$color <- results$colors
V(network)$color <- "greenyellow"
scale_factor <- 0.1
par(mar=c(0,0,0,0))
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_greenyellow.pdf",width = 25,height = 25)
#plot(network, vertex.color="yellow", vertex.label="",vertex.size=2,layout=layout_in_circle(network), edge.arrow.size = 0.2)
#plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=2.5, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
magenta_top20_gene_list <- magenta_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="magenta"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
#}#yellow module we have 350 genes!!!
...
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 350
if(!is.na(top.n.edges) & nrow(adj_mat) >= 100){
  adjacency.threshold = sort(as.numeric(adj_mat), decreasing=T)[(top.n.edges*2)+nrow(adj_mat)]
}
if(nrow(adj_mat) < 100)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.147451379931329
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 215
...
#results <- blockwiseModules(data, power=6, TOMType="unsigned", networkType="unsigned")
#V(network)$color <- results$colors
V(network)$color <- "magenta"
scale_factor <- 0.1
par(mar=c(0,0,0,0))
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_magenta.pdf",width = 25,height = 25)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
kmes<-data.frame(genes = gs, cols = cols, kme = kmes)
head(kmes)
lightcyan_kmes <- kmes[kmes$cols=="lightcyan",]
head(lightcyan_kmes)
lightcyan_kmes <-lightcyan_kmes[order(-lightcyan_kmes$kme),]
lightcyan_top20_gene_list <- lightcyan_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="lightcyan"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
#}#yellow module we have 118 genes!!!
...
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 118
if(!is.na(top.n.edges) & nrow(adj_mat) >= 100){
  adjacency.threshold = sort(as.numeric(adj_mat), decreasing=T)[(top.n.edges*2)+nrow(adj_mat)]
}
if(nrow(adj_mat) < 100)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.0110144896118156
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 117
...
#results <- blockwiseModules(data, power=6, TOMType="unsigned", networkType="unsigned")
#V(network)$color <- results$colors
V(network)$color <- "lightcyan"
scale_factor <- 0.1
par(mar=c(0,0,0,0))
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_lightcan.pdf",width = 25,height = 25)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
kmes<-data.frame(genes = gs, cols = cols, kme = kmes)
head(kmes)
midnightblue_kmes <- kmes[kmes$cols=="midnightblue",]
head(midnightblue_kmes)
midnightblue_kmes <-midnightblue_kmes[order(-midnightblue_kmes$kme),]
midnightblue_top20_gene_list <- midnightblue_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="midnightblue"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.021367240414569
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 130
...
#results <- blockwiseModules(data, power=6, TOMType="unsigned", networkType="unsigned")
#V(network)$color <- results$colors
V(network)$color <- "midnightblue"
scale_factor <- 0.1
par(mar=c(0,0,0,0))
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_midnightblue.pdf",width = 25,height = 25)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
black_top20_gene_list <- black_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="black"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
#}#yellow module we have 834 genes!!!
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.485454752123474
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 277
...
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_black.pdf",width = 25,height = 25)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
kmes<-data.frame(genes = gs, cols = cols, kme = kmes)
head(kmes)
salmon_kmes <- kmes[kmes$cols=="salmon",]
head(salmon_kmes)
salmon_kmes <-salmon_kmes[order(-salmon_kmes$kme),]
salmon_top20_gene_list <- salmon_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="salmon"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
...
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 174
if(!is.na(top.n.edges) & nrow(adj_mat) >= 100){
  adjacency.threshold = sort(as.numeric(adj_mat), decreasing=T)[(top.n.edges*2)+nrow(adj_mat)]
}
if(nrow(adj_mat) < 100)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.0620871212658785
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
if(verbose) cat("removing unconnected nodes\n")
adj_mat<-adj_mat[rs>min.edge,rs>min.edge]
message("number of genes present = ", nrow(adj_mat))#number of genes present = 159
...
#results <- blockwiseModules(data, power=6, TOMType="unsigned", networkType="unsigned")
#V(network)$color <- results$colors
V(network)$color <- "salmon"
scale_factor <- 0.1
par(mar=c(0,0,0,0))
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_salmon.pdf",width = 25,height = 25)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
...
min.edge = 2
#head(adj_mat)
message("number of genes present = ", nrow(adj_mat))#number of genes present = 138
if(!is.na(top.n.edges) & nrow(adj_mat) >= 100){
  adjacency.threshold = sort(as.numeric(adj_mat), decreasing=T)[(top.n.edges*2)+nrow(adj_mat)]
}
if(nrow(adj_mat) < 100)
...
message("adjacency.threshold = ", adjacency.threshold)#adjacency.threshold = 0.0467523726341212
adj_mat[adj_mat > adjacency.threshold] <- 1
adj_mat[adj_mat < adjacency.threshold] <- 0
diag(adj_mat) <- 0
rs<-rowSums(adj_mat)
...
kmes<-data.frame(genes = gs, cols = cols, kme = kmes)
head(kmes)
grey60_kmes <- kmes[kmes$cols=="grey60",]
head(grey60_kmes)
grey60_kmes <-cyan_kmes[order(-grey60_kmes$kme),]
grey60_top20_gene_list <- cyan_kmes[1:20,]$genes

gs<-kmes$genes[kmes$cols=="grey60"]
#cols<-cols[kmes$kme>=kME.threshold]
datExpr = datExpr0[,gs]
head(datExpr[,1:8])
ncol(datExpr)
...
#results <- blockwiseModules(data, power=6, TOMType="unsigned", networkType="unsigned")
#V(network)$color <- results$colors
V(network)$color <- "grey60"
scale_factor <- 0.1
par(mar=c(0,0,0,0))
# remove unconnected nodes
network <- delete.vertices(network, degree(network)==0)
pdf(file = "/global/u2/l/llei2019/Brachypodium/Sylvaticum/RNAseq/network_all_abiotic_VST#_grey60.pdf",width = 25,height = 25)
plot(network,pch=19,arrow.mode=0,mark.border=NA, edge.color = "#d1d1e0", vertex.label="",vertex.size=V(network)$vertex_degree*scale_factor, layout=layout.fruchterman.reingold(network), edge.arrow.size = 0.2)
dev.off()
